...
void main() {
  group('MetricsUserMenuCard', () {
    const testActiveColor = Colors.amberAccent;
    ...
    const testTheme = MetricsThemeData(
      userMenuTheme: UserMenuThemeData(
        ...
        primaryTextStyle: testTextStyle,
      ),
    );
    ...
    testWidgets(
      "applies the border color from the user menu theme to the flutter switch widget",
      (WidgetTester tester) async {
        await tester.pumpWidget(const _MetricsUserMenuCardTestbed(
          theme: testTheme,
        ));

        final switchWidget = tester.widget<FlutterSwitch>(
          find.byType(FlutterSwitch),
        );

        expect(switchWidget.activeColor, equals(testActiveColor));
      },
    );
